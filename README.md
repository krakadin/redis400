...
This project provides Redis utility functions for IBM i, enabling seamless interaction with a Redis server through SQL User-Defined Functions (UDFs). It includes two main functions:
...
2. **Compile the C Modules into ILE Modules**:
   - The C source files (`redisget.c`, `redisset.c`, and `redisutils.c`) are compiled into ILE modules:
     - `REDISGET`
     - `REDISSET`
     - `REDISUTILS`
...
RUNSQL SQL('SELECT _FROM QSYS2.SYSFUNCS WHERE SPECIFIC_NAME = ''REDIS_GET''')
RUNSQL SQL('SELECT_ FROM QSYS2.SYSFUNCS WHERE SPECIFIC_NAME = ''REDIS_SET''')
...
```sql
SELECT REDIS_GET('my_key') AS value FROM SYSIBM.SYSDUMMY1;
```

Using REDIS_SET

```sql
VALUES REDIS_SET('my_key', 'my_value');
```

### Notes

Ensure the Redis server is running and accessible at `127.0.0.1:6379` (configurable in `redisset.c`).
...
- Roadmap: Future enhancements may include additional Redis commands (e.g., `DEL`, `INCR`), Python integration, and improved error handling.

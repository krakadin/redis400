...
redis_get.func:
	-system "RUNSQL SQL('CREATE OR REPLACE FUNCTION $(TGT_LIB).REDIS_GET (KEY VARCHAR(255)) RETURNS VARCHAR(32740) LANGUAGE C SPECIFIC REDIS_GET NOT DETERMINISTIC NO SQL RETURNS NULL ON NULL INPUT DISALLOW PARALLEL NOT FENCED EXTERNAL NAME ''$(TGT_LIB)/REDISILE(getRedisValue)'' PARAMETER STYLE DB2SQL') COMMIT(*NONE)"

# Create or replace the SQL function for redis_set
redis_set.func:
	-system "RUNSQL SQL('CREATE OR REPLACE FUNCTION $(TGT_LIB).REDIS_SET (KEY VARCHAR(255), VALUE VARCHAR(32740)) RETURNS VARCHAR(128) LANGUAGE C SPECIFIC REDIS_SET NOT DETERMINISTIC NO SQL RETURNS NULL ON NULL INPUT DISALLOW PARALLEL NOT FENCED EXTERNAL NAME ''$(TGT_LIB)/REDISILE(setRedisValue)'' PARAMETER STYLE DB2SQL') COMMIT(*NONE)"

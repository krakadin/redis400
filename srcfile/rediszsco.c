 ...
 * Date: 2025-02-22
 * Description: Implementation of the Redis ZSCORE function for IBM i.
 *              This function gets the score of a member in a sorted set.
...
/**********************************************************************/
/* SQL External Function: ZSCORE */
/**********************************************************************/

/**
 * Function: zscoreRedisSSet
 * Description: SQL external function to get the score of a member in a Redis sorted set.
 * Parameters:
 *   - key: Input Redis key (VARCHAR(255), EBCDIC).
 *   - member: Input member name (VARCHAR(255), EBCDIC).
 *   - value: Output score value (VARCHAR(50), EBCDIC).
	...
	SQLUDF_NULLIND *nullind)	 // Additional null indicators for DB2SQL
{
	...
	*valueInd = 0;
	...
	// Format Redis ZSCORE command in EBCDIC
	ebcdic_send_buf[0] = '\0';
	...
	// Calculate and format member length in EBCDIC (up to 255 bytes)
	...
	// Build ZSCORE command in EBCDIC:
	// 	"*3\r\n$6\r\nZSCORE\r\n$<key_len>\r\n<key>\r\n$<member_len>\r\n<member>\r\n"
	strcat(ebcdic_send_buf, "\x5C\xF3\x0D\x25\x5B\xF6\x0D\x25\xE9\xE2\xC3\xD6\xD9\xC5\x0D\x25"); // EBCDIC "*3\r\n$6\r\nZSCORE\r\n"
	...
	// Convert EBCDIC ZSCORE command to ASCII before sending
	...
	else if (extract_result == -2) // (nil) - member not found in sorted set
	{
...
#pragma linkage(zscoreRedisSSet, OS)

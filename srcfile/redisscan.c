 ...
 * Date: 2025-02-22
 * Description: Implementation of the Redis SCAN function for IBM i.
 *              Iteratively scans keys matching a pattern using cursor-based
 *              pagination. Safe for production use (non-blocking).
 *              Returns "cursor|key1,key2,key3" format.
 *              Cursor "0" means scan is complete.
 *              Example: SCAN('0', 'user:*', 100) → "17|user:1,user:2"
 *                       SCAN('17', 'user:*', 100) → "0|user:3,user:4"
...
/**********************************************************************/
/* Helper: Parse nested RESP array for SCAN                            */
/* SCAN returns: *2\r\n$<clen>\r\n<cursor>\r\n*N\r\n...keys...        */
/* Output format: "cursor|key1,key2,key3"                              */
/**********************************************************************/

static int parse_scan_response(char *ebcdic_response, char *output, size_t output_size)
{
    ...
    if (outer_count != 2)
        return -1; // SCAN always returns 2 elements
    ...
    // Step 4: Parse inner array (*N) for keys
    if (*pos != 0x5C) // EBCDIC '*'
        return -1;
    pos++;
    ...
    int key_count = atoi(count_buf);
    pos = crlf + 2; // Skip past \r\n

    if (key_count <= 0)
    {
        // No keys in this iteration
        output[out_offset] = '\0';
        return 0; // Success, 0 keys
    }

    // Step 5: Parse each key
    int elem_count = 0;
    int i;
    for (i = 0; i < key_count; i++)
    {
        ...
        // Append comma separator (if not first key)
        if (elem_count > 0)
        {
            if (out_offset + 1 >= output_size)
                break;
...
/**********************************************************************/
/* SQL External Function: SCAN                                         */
/**********************************************************************/

void SQL_API_FN scanRedisKeys(
    ...
    SQLUDF_VARCHAR *value,        // Output: cursor|keys (VARCHAR(16370), EBCDIC)
    ...
    SQLUDF_NULLIND *nullind)
{
    ...
    if (*cursorInd < 0 || *patternInd < 0 || *countInd < 0)
    {
    ...
    *valueInd = 0;
    ...
    // Build SCAN command:
    // "*6\r\n$4\r\nSCAN\r\n$<clen>\r\n<cursor>\r\n$5\r\nMATCH\r\n$<plen>\r\n<pattern>\r\n$5\r\nCOUNT\r\n$<cnt_len>\r\n<count>\r\n"
    strcat(ebcdic_send_buf, "\x5C\xF6\x0D\x25\x5B\xF4\x0D\x25\xE2\xC3\xC1\xD5\x0D\x25"); // *6\r\n$4\r\nSCAN\r\n
    ...
    int parse_result = parse_scan_response(ebcdic_response, value, 16370);

    if (parse_result >= 0)
    {
        *valueInd = 0; // Success (0 keys is valid for SCAN)
    }
    else
    {
        strncpy(sqlstate, "38909", 5);
        snprintf(msgtext, 70, "Failed to parse SCAN response");
        *valueInd = -1;
    }
...
#pragma linkage(scanRedisKeys, OS)

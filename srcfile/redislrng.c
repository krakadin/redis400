 ...
 * Date: 2025-02-22
 * Description: Implementation of the Redis LRANGE function for IBM i.
 *              Returns a range of elements from a Redis list as a
...
/**********************************************************************/
/* Helper: Parse RESP array for LRANGE                                */
/**********************************************************************/

static int parse_lrange_response(char *ebcdic_response, char *output, size_t output_size)
{
...
/**********************************************************************/
/* SQL External Function: LRANGE                                      */
/**********************************************************************/

void SQL_API_FN lrangeRedisList(
    ...
    SQLUDF_VARCHAR *value,        // Output: comma-separated elements (VARCHAR(16370), EBCDIC)
    ...
    SQLUDF_NULLIND *nullind)
{
    ...
    *valueInd = 0;
    ...
    // "*4\r\n$6\r\nLRANGE\r\n$<key_len>\r\n<key>\r\n$<start_len>\r\n<start>\r\n$<stop_len>\r\n<stop>\r\n"
    strcat(ebcdic_send_buf, "\x5C\xF4\x0D\x25\x5B\xF6\x0D\x25\xD3\xD9\xC1\xD5\xC7\xC5\x0D\x25"); // *4\r\n$6\r\nLRANGE\r\n
    ...
    int parse_result = parse_lrange_response(ebcdic_response, value, 16370);
    ...
    else if (parse_result == -2 || parse_result == 0)
    {
        ...
        strncpy(msgtext, "List is empty or range out of bounds", 70);
        *valueInd = -1;
    }
    else
    {
        strncpy(sqlstate, "38909", 5);
        snprintf(msgtext, 70, "Failed to parse LRANGE response");
        *valueInd = -1;
    }
...
#pragma linkage(lrangeRedisList, OS)

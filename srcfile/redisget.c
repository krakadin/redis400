    ...
    SQLUDF_NULLIND *nullind)  // Additional null indicators for DB2SQL
{
    ...
    *valueInd = 0;
    ...
    else if (extract_result == -2) // Specific error (timeout or key not found)
    {
        strncpy(sqlstate, "02000", 5); // No data found (appropriate for Redis timeout or key not found)
        strncpy(msgtext, "Redis operation timed out or key not found", 70);
        *valueInd = -1; // Indicate null value
    }
    else // All other errors (-1 or unexpected values)
    {

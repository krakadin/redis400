 ...
 * Date: 2025-02-22
 * Description: Implementation of the Redis SSCAN function for IBM i.
 *              Cursor-based scanning of set members matching a pattern.
 *              Safe for production use (non-blocking).
 *              Returns "cursor|member1,member2,member3" format.
 *              Cursor "0" means scan is complete.
 *              Example: SSCAN('myset', '0', 'user:*', 100) → "17|user:1,user:2"
 *                       SSCAN('myset', '17', 'user:*', 100) → "0|user:3,user:4"
...
/**********************************************************************/
/* Helper: Parse nested RESP array for SSCAN                           */
/* SSCAN returns: *2\r\n$<clen>\r\n<cursor>\r\n*N\r\n...members...    */
/* Output format: "cursor|member1,member2,member3"                     */
/**********************************************************************/

static int parse_sscan_response(char *ebcdic_response, char *output, size_t output_size)
{
    ...
    if (outer_count != 2)
        return -1; // SSCAN always returns 2 elements
    ...
    // Step 4: Parse inner array (*N) for members
    if (*pos != 0x5C) // EBCDIC '*'
        return -1;
    pos++;
    ...
    int member_count = atoi(count_buf);
    pos = crlf + 2; // Skip past \r\n

    if (member_count <= 0)
    {
        // No members in this iteration
        output[out_offset] = '\0';
        return 0; // Success, 0 members
    }

    // Step 5: Parse each member
    int elem_count = 0;
    int i;
    for (i = 0; i < member_count; i++)
    {
        ...
        // Append comma separator (if not first member)
        if (elem_count > 0)
        {
            if (out_offset + 1 >= output_size)
                break;
        ...
        // Check if we have room for this member
...
/**********************************************************************/
/* SQL External Function: SSCAN                                        */
/**********************************************************************/

void SQL_API_FN sscanRedisSet(
    ...
    SQLUDF_VARCHAR *value,        // Output: cursor|member1,member2,member3 (VARCHAR(16370), EBCDIC)
    ...
    SQLUDF_NULLIND *nullind)
{
    ...
    *valueInd = 0;
    ...
    // Build SSCAN command:
    // "*7\r\n$5\r\nSSCAN\r\n$<klen>\r\n<key>\r\n$<clen>\r\n<cursor>\r\n$5\r\nMATCH\r\n$<plen>\r\n<pattern>\r\n$5\r\nCOUNT\r\n$<cnt_len>\r\n<count>\r\n"
    strcat(ebcdic_send_buf, "\x5C\xF7\x0D\x25\x5B\xF5\x0D\x25\xE2\xE2\xC3\xC1\xD5\x0D\x25"); // *7\r\n$5\r\nSSCAN\r\n
    ...
    int parse_result = parse_sscan_response(ebcdic_response, value, 16370);

    if (parse_result >= 0)
    {
        *valueInd = 0; // Success (0 members is valid for SSCAN)
    }
    else
    {
        strncpy(sqlstate, "38909", 5);
        snprintf(msgtext, 70, "Failed to parse SSCAN response");
        *valueInd = -1;
    }
...
#pragma linkage(sscanRedisSet, OS)

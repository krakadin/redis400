 ...
 * Date: 2025-02-22
 * Description: Implementation of the Redis SREM function for IBM i.
 *              This function removes a member from a Redis set.
 *              Returns 1 if the member was removed, 0 if not a member.
...
/**********************************************************************/
/* SQL External Function: SREM */
/**********************************************************************/

/**
 * Function: sremRedisSet
 * Description: SQL external function to remove a member from a Redis set.
 * Parameters:
 *   - key: Input Redis key (VARCHAR(255), EBCDIC).
 *   - member: Input set member (VARCHAR(16370), EBCDIC).
 *   - result: Output number of members removed (BIGINT, 1=removed, 0=not member).
	...
	SQLUDF_NULLIND *nullind)	 // Additional null indicators for DB2SQL
{
	...
	*resultInd = 0;
	...
	// Format Redis SREM command in EBCDIC
	ebcdic_send_buf[0] = '\0';
	...
	// Build SREM command in EBCDIC:
	// 	"*3\r\n$4\r\nSREM\r\n$<key_len>\r\n<key>\r\n$<member_len>\r\n<member>\r\n"
	strcat(ebcdic_send_buf, "\x5C\xF3\x0D\x25\x5B\xF4\x0D\x25\xE2\xD9\xC5\xD4\x0D\x25"); // EBCDIC "*3\r\n$4\r\nSREM\r\n"
	...
	// Convert EBCDIC SREM command to ASCII before sending
	...
	{
		// Parse the integer value from the payload (1=removed, 0=not member)
		*result = atol(payload);
		*resultInd = 0;
	}
	else
...
#pragma linkage(sremRedisSet, OS)

 ...
 * Date: 2025-02-22
 * Description: Implementation of the Redis HSET function for IBM i.
 *              This function sets a field in a Redis hash.
 *              Returns the number of fields that were added (1 if new, 0 if updated).
...
/**********************************************************************/
/* SQL External Function: HSET */
/**********************************************************************/

/**
 * Function: hsetRedisValue
 * Description: SQL external function to set a field in a Redis hash.
 * Parameters:
 *   - key: Input Redis key (VARCHAR(255), EBCDIC).
 *   - field: Input hash field name (VARCHAR(255), EBCDIC).
 *   - value: Input hash field value (VARCHAR(16370), EBCDIC).
 *   - result: Output number of fields added (BIGINT, 1=new, 0=updated).
 ...
 */
void SQL_API_FN hsetRedisValue(
	SQLUDF_VARCHAR *key,		 // Input: Redis key (VARCHAR(255), EBCDIC)
	SQLUDF_VARCHAR *field,		 // Input: Hash field name (VARCHAR(255), EBCDIC)
	SQLUDF_VARCHAR *value,		 // Input: Hash field value (VARCHAR(16370), EBCDIC)
	SQLUDF_BIGINT *result,		 // Output: fields added (BIGINT, 1=new, 0=updated)
	...
	SQLUDF_NULLIND *nullind)	 // Additional null indicators for DB2SQL
{
	...
	char ebcdic_key_len[10] = {0}, ebcdic_field_len[10] = {0}, ebcdic_value_len[10] = {0};
	int len, total_len = 0;
	...
	if (*keyInd == -1 || *fieldInd == -1 || *valueInd == -1)
	{
		strncpy(sqlstate, "38001", 5);
		strncpy(msgtext, "Input key, field, or value is NULL", 70);
		*resultInd = -1;
		return;
	}
	...
	*resultInd = 0;
	...
	// Format Redis HSET command in EBCDIC
	ebcdic_send_buf[0] = '\0';
	...
	// Build HSET command in EBCDIC:
	// 	"*4\r\n$4\r\nHSET\r\n$<key_len>\r\n<key>\r\n$<field_len>\r\n<field>\r\n$<value_len>\r\n<value>\r\n"
	strcat(ebcdic_send_buf, "\x5C\xF4\x0D\x25\x5B\xF4\x0D\x25\xC8\xE2\xC5\xE3\x0D\x25"); // EBCDIC "*4\r\n$4\r\nHSET\r\n"
	...
	// Convert EBCDIC HSET command to ASCII before sending
	...
	{
		// Parse the integer value from the payload (1=new field, 0=updated existing)
		*result = atol(payload);
		*resultInd = 0;
	}
	else
...
#pragma linkage(hsetRedisValue, OS)

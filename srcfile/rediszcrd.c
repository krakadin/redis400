 ...
 * Date: 2025-02-22
 * Description: Implementation of the Redis ZCARD function for IBM i.
 *              This function returns the cardinality (number of members)
 *              of a Redis sorted set. Returns 0 if the key does not exist.
...
/**********************************************************************/
/* SQL External Function: ZCARD */
/**********************************************************************/

void SQL_API_FN zcardRedisSSet(
    SQLUDF_VARCHAR *key,      // Input: Redis key (VARCHAR(255), EBCDIC)
    SQLUDF_BIGINT *value,     // Output: sorted set cardinality (BIGINT)
    ...
    SQLUDF_NULLIND *nullind)  // Additional null indicators for DB2SQL
{
    ...
    *valueInd = 0;
    ...
    // Format Redis ZCARD command in EBCDIC
    ebcdic_send_buf[0] = '\0';
    ...
    // Build ZCARD command: "*2\r\n$5\r\nZCARD\r\n$<key_len>\r\n<key>\r\n"
    strcat(ebcdic_send_buf, "\x5C\xF2\x0D\x25\x5B\xF5\x0D\x25\xE9\xC3\xC1\xD9\xC4\x0D\x25"); // *2\r\n$5\r\nZCARD\r\n
...
#pragma linkage(zcardRedisSSet, OS)

 ...
 * Date: 2025-02-22
 * Description: Implementation of the Redis TYPE function for IBM i.
 *              This function returns the type of the value stored at key.
 *              Returns one of: "string", "list", "set", "hash", "zset",
...
/**********************************************************************/
/* SQL External Function: TYPE */
/**********************************************************************/

void SQL_API_FN typeRedisKey(
    SQLUDF_VARCHAR *key,      // Input: Redis key (VARCHAR(255), EBCDIC)
    SQLUDF_VARCHAR *value,    // Output: key type (VARCHAR(20), EBCDIC)
...
{
    int sockfd;
    char ebcdic_send_buf[512], ascii_send_buf[512], recv_buf[256], ebcdic_payload[256];
    ...
    *valueInd = 0;
    ...
    // Format Redis TYPE command in EBCDIC
    ebcdic_send_buf[0] = '\0';
    ...
    // Build TYPE command: "*2\r\n$4\r\nTYPE\r\n$<key_len>\r\n<key>\r\n"
    strcat(ebcdic_send_buf, "\x5C\xF2\x0D\x25\x5B\xF4\x0D\x25\xE3\xE8\xD7\xC5\x0D\x25"); // *2\r\n$4\r\nTYPE\r\n
    ...
    if (extract_result == 0)
    {
        ...
        else
        {
            ...
            strcpy(msgtext, "Response exceeds maximum length");
            *valueInd = -1;
        }
    }
    else
...
#pragma linkage(typeRedisKey, OS)

 ...
 * Date: 2025-02-22
 * Description: Implementation of the Redis HDEL function for IBM i.
 *              This function deletes a field from a Redis hash.
 *              Returns the number of fields that were removed (1 or 0).
...
/**********************************************************************/
/* SQL External Function: HDEL */
/**********************************************************************/

/**
 * Function: hdelRedisField
 * Description: SQL external function to delete a field from a Redis hash.
 * Parameters:
 *   - key: Input Redis key (VARCHAR(255), EBCDIC).
 *   - field: Input hash field name (VARCHAR(255), EBCDIC).
 *   - result: Output number of fields removed (BIGINT, 1=deleted, 0=not found).
	...
	SQLUDF_NULLIND *nullind)	 // Additional null indicators for DB2SQL
{
	...
	*resultInd = 0;
	...
	// Format Redis HDEL command in EBCDIC
	ebcdic_send_buf[0] = '\0';
	...
	// Build HDEL command in EBCDIC:
	// 	"*3\r\n$4\r\nHDEL\r\n$<key_len>\r\n<key>\r\n$<field_len>\r\n<field>\r\n"
	strcat(ebcdic_send_buf, "\x5C\xF3\x0D\x25\x5B\xF4\x0D\x25\xC8\xC4\xC5\xD3\x0D\x25"); // EBCDIC "*3\r\n$4\r\nHDEL\r\n"
	...
	// Convert EBCDIC HDEL command to ASCII before sending
...
#pragma linkage(hdelRedisField, OS)

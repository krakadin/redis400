 ...
 * Date: 2025-02-22
 * Description: Implementation of the Redis PERSIST function for IBM i.
 *              This function removes the existing timeout on a key, turning
 *              the key from volatile (with an expiration) to persistent.
 *              Returns 1 if the timeout was removed, 0 if the key does not
 *              exist or does not have an associated timeout.
...
/**********************************************************************/
/* SQL External Function: PERSIST */
/**********************************************************************/

void SQL_API_FN persistRedisKey(
    SQLUDF_VARCHAR *key,      // Input: Redis key (VARCHAR(255), EBCDIC)
    SQLUDF_BIGINT *value,     // Output: 1=timeout removed, 0=no timeout or key missing (BIGINT)
    ...
    SQLUDF_NULLIND *nullind)  // Additional null indicators for DB2SQL
{
    ...
    *valueInd = 0;
    ...
    // Format Redis PERSIST command in EBCDIC
    ebcdic_send_buf[0] = '\0';
    ...
    // Build PERSIST command: "*2\r\n$7\r\nPERSIST\r\n$<key_len>\r\n<key>\r\n"
    strcat(ebcdic_send_buf, "\x5C\xF2\x0D\x25\x5B\xF7\x0D\x25\xD7\xC5\xD9\xE2\xC9\xE2\xE3\x0D\x25"); // *2\r\n$7\r\nPERSIST\r\n
...
#pragma linkage(persistRedisKey, OS)

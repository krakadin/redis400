 ...
 * Date: 2025-02-22
 * Description: Implementation of the Redis EXISTS function for IBM i.
 *              This function checks if a key exists in a Redis server.
...
/**
 * Function: existsRedisKey
 * Description: SQL external function to check if a key exists in Redis.
 * Parameters:
 *   - key: Input Redis key (VARCHAR(255), EBCDIC).
 *   - value: Output result (BIGINT, 1=exists, 0=not exists).
 ...
 */
void SQL_API_FN existsRedisKey(
    SQLUDF_VARCHAR *key,      // Input: Redis key (VARCHAR(255), EBCDIC)
    SQLUDF_BIGINT *value,     // Output: 1 if exists, 0 if not (BIGINT)
    ...
    SQLUDF_NULLIND *nullind)  // Additional null indicators for DB2SQL
{
    ...
    *valueInd = 0;
    ...
    // Format Redis EXISTS command in EBCDIC
    ebcdic_send_buf[0] = '\0';
    ...
    // Build EXISTS command in EBCDIC: "*2\r\n$6\r\nEXISTS\r\n$<key_len>\r\n<key>\r\n"
    strcat(ebcdic_send_buf, "\x5C\xF2\x0D\x25\x5B\xF6\x0D\x25\xC5\xE7\xC9\xE2\xE3\xE2\x0D\x25"); // EBCDIC "*2\r\n$6\r\nEXISTS\r\n"
    ...
    // Convert EBCDIC EXISTS command to ASCII before sending
    ascii_send_buf[0] = '\0';
...
#pragma linkage(existsRedisKey, OS)

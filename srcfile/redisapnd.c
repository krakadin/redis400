 ...
 * Date: 2025-02-22
 * Description: Implementation of the Redis APPEND function for IBM i.
 *              This function appends a value to an existing string in Redis.
 *              If the key does not exist, it is created and set to the value.
...
/**********************************************************************/
/* SQL External Function: APPEND */
/**********************************************************************/

/**
 * Function: appendRedisValue
 * Description: SQL external function to append a value to a key in Redis.
 *              Returns the length of the string after the append operation.
 * Parameters:
 *   - key: Input Redis key (VARCHAR(255), EBCDIC).
 *   - value: Input Redis value (VARCHAR(16370), EBCDIC).
 *   - result: Output string length after append (BIGINT).
...
void SQL_API_FN appendRedisValue(
	...
	SQLUDF_BIGINT *result,		 // Output: string length after append (BIGINT)
	...
	SQLUDF_NULLIND *nullind)	 // Additional null indicators for DB2SQL
{
	...
	*resultInd = 0;
	...
	// Format Redis APPEND command in EBCDIC
	ebcdic_send_buf[0] = '\0';
	...
	// Build APPEND command in EBCDIC:
	// 	"*3\r\n$6\r\nAPPEND\r\n$<key_len>\r\n<key>\r\n$<value_len>\r\n<value>\r\n"
	strcat(ebcdic_send_buf, "\x5C\xF3\x0D\x25\x5B\xF6\x0D\x25\xC1\xD7\xD7\xC5\xD5\xC4\x0D\x25"); // EBCDIC "*3\r\n$6\r\nAPPEND\r\n"
	...
	// Convert EBCDIC APPEND command to ASCII before sending
	...
	{
		// Parse the integer value from the payload (string length after append)
		*result = atol(payload); // Convert the payload to a BIGINT
		*resultInd = 0;
	}
	else
	{

 ...
 * Date: 2025-02-22
 * Description: Implementation of the Redis SISMEMBER function for IBM i.
 *              This function checks if a member exists in a Redis set.
...
/**********************************************************************/
/* SQL External Function: SISMEMBER */
/**********************************************************************/

/**
 * Function: sismemberRedisSet
 * Description: SQL external function to check if a member exists in a Redis set.
 * Parameters:
 *   - key: Input Redis key (VARCHAR(255), EBCDIC).
 *   - member: Input set member (VARCHAR(16370), EBCDIC).
 *   - result: Output result (BIGINT, 1=member exists, 0=not member).
 ...
 */
void SQL_API_FN sismemberRedisSet(
	SQLUDF_VARCHAR *key,		 // Input: Redis key (VARCHAR(255), EBCDIC)
	SQLUDF_VARCHAR *member,		 // Input: Set member (VARCHAR(16370), EBCDIC)
	SQLUDF_BIGINT *result,		 // Output: 1 if member exists, 0 if not (BIGINT)
...
{
	int sockfd;
	char ebcdic_send_buf[17000], ascii_send_buf[17000], recv_buf[1024], ebcdic_payload[1024];
	...
	*resultInd = 0;
	...
	// Format Redis SISMEMBER command in EBCDIC
	ebcdic_send_buf[0] = '\0';
	...
	// Build SISMEMBER command in EBCDIC:
	// 	"*3\r\n$9\r\nSISMEMBER\r\n$<key_len>\r\n<key>\r\n$<member_len>\r\n<member>\r\n"
	strcat(ebcdic_send_buf, "\x5C\xF3\x0D\x25\x5B\xF9\x0D\x25\xE2\xC9\xE2\xD4\xC5\xD4\xC2\xC5\xD9\x0D\x25"); // EBCDIC "*3\r\n$9\r\nSISMEMBER\r\n"
	...
	// Convert EBCDIC SISMEMBER command to ASCII before sending
	...
	// Send SISMEMBER command to Redis
	...
	{
		// Parse the integer value from the payload (1=member exists, 0=not member)
		*result = atol(payload);
		*resultInd = 0;
	}
	else
...
#pragma linkage(sismemberRedisSet, OS)

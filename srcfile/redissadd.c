 ...
 * Date: 2025-02-22
 * Description: Implementation of the Redis SADD function for IBM i.
 *              This function adds a member to a Redis set.
 *              Returns 1 if the member was added, 0 if already a member.
...
/**********************************************************************/
/* SQL External Function: SADD */
/**********************************************************************/

/**
 * Function: saddRedisSet
 * Description: SQL external function to add a member to a Redis set.
 * Parameters:
 *   - key: Input Redis key (VARCHAR(255), EBCDIC).
 *   - member: Input set member (VARCHAR(16370), EBCDIC).
 *   - result: Output number of members added (BIGINT, 1=added, 0=already member).
	...
	SQLUDF_NULLIND *nullind)	 // Additional null indicators for DB2SQL
{
	...
	*resultInd = 0;
	...
	// Format Redis SADD command in EBCDIC
	ebcdic_send_buf[0] = '\0';
	...
	// Build SADD command in EBCDIC:
	// 	"*3\r\n$4\r\nSADD\r\n$<key_len>\r\n<key>\r\n$<member_len>\r\n<member>\r\n"
	strcat(ebcdic_send_buf, "\x5C\xF3\x0D\x25\x5B\xF4\x0D\x25\xE2\xC1\xC4\xC4\x0D\x25"); // EBCDIC "*3\r\n$4\r\nSADD\r\n"
	...
	// Convert EBCDIC SADD command to ASCII before sending
	...
	{
		// Parse the integer value from the payload (1=added, 0=already member)
		*result = atol(payload);
		*resultInd = 0;
	}
	else
...
#pragma linkage(saddRedisSet, OS)
